...
## Review — `review.go` (Phase 1, step 7) -- built (#56)
...
| Method | Path | Permission | Purpose |
|---|---|---|---|
| GET | `/decks/{id}/review` | `can_view` + `can_study` | Reviewer page. First batch (20 cards), the precomputed 4-rating outcome per card, and the user's FSRS params are rendered inline in the response — no separate request for card 1 (§6). On a paced deck with a locked lesson still pending, the empty state also names the date it opens (#243) |
