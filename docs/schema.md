...
deck_access deck_id, user_id, created_at,
            ...
            can_view_progress bool, can_view_flags bool
            -- PRIMARY KEY (deck_id, user_id)
            -- the ONLY thing that makes a deck reachable by a second user
            -- eight independent per-(user, deck) permissions, not a role enum -- see below

card_flags  id, card_id, deck_id, flagged_by_user_id, comment, status,
            created_at, resolved_at, resolved_by_user_id
            -- a student's feedback on one card (#207), addressed to whoever holds
            -- can_view_flags on its deck; status is 'open' | 'resolved'
            -- UNIQUE (card_id, flagged_by_user_id) WHERE status = 'open'
            --   <- resubmitting while one is still open replaces it, no duplicates
            -- deck_id is denormalised from cards.deck_id for permission-scoped queries
```
...
`deck_access` grants eight independent permissions per `(user_id, deck_id)` — a row can hold any
...
| `can_view_flags` | Read and resolve `card_flags` rows on this one deck (#207) — students' flagged-card comments, `GET /decks/{id}/flags`. Deliberately separate from `can_view_progress`: a named comment is individual-level disclosure, which that flag's scope (aggregate-only) was written to exclude |

`can_view` is a practical prerequisite for the other seven to mean anything, but that's an
...
A deck's creator gets all eight flags on creation. A personal, single-user deck is just the
...
| `card_flags.card_id → cards` | CASCADE | Unlike `review_log.card_id` (no FK, above), a flag carries no protected-training-data status (§2.5 doesn't apply) — if the card is deleted or regenerated by a note edit, the simplest thing is for the flag to go with it (#207). |
