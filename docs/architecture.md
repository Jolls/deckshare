...
| Release pacing | No such concept. A deck's content is available the moment it exists — the one person who owns the collection is the one person studying it, and pacing is just a daily new-card limit they set for themselves | `notes.release_day` plus `decks.preset.calendar` gate never-seen cards on the class meeting their lesson belongs to, resolved against each student's own study day ([#242](https://github.com/Jolls/deckshare/issues/242), part of [#238](https://github.com/Jolls/deckshare/issues/238); docs/schema.md). Traces to the §2.1 seam: one deck's content, many learners on schedules the deck's *author* sets, is not a thing a single-user collection can express. Orthogonal to `new.perDay` and deliberately so — the gate says which cards are eligible, the limit says how many. Deferred, both additive: a `cards.release_day` override via `COALESCE`, and round-tripping a `lesson::N` note tag through `.apkg` |
