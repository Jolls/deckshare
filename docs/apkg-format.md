...
- **`revlog.factor` and `revlog.lastIvl`** (`IrReview.Factor`, `IrReview.LastIntervalSeconds`) are
  read from an imported `.apkg`'s `revlog` table but never persisted anywhere in DeckShare's schema, so
...
- **`col.crt`** (the collection's creation timestamp) is not stored by DeckShare at all — a deck has no
...
DeckShare doesn't schedule from, or a value Anki's own importer recomputes/tolerates a default for) —
...
DeckShare's `DefaultDesiredRetention` (0.9) instead of whatever retention Anki was actually scheduling
...
setting mid-collection, but DeckShare schedules per-(user, deck) (invariant §2.3), so only one
