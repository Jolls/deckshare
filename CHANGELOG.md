...
- The reviewer's visible card never carried the CSS scope class, so a note type's custom CSS
  ...
  ([#194](https://github.com/Jolls/deckshare/issues/194)).
...
- Deck managers can reset a collaborator's scheduling progress on a deck from the access page;
  ...
  ([#189](https://github.com/Jolls/deckshare/issues/189)).

## [0.2.22] - 2026-09-03

### Added
- The decks list shows a 1-person icon for private decks and a 2-person icon for decks shared
  with another user ([#190](https://github.com/Jolls/deckshare/issues/190)).
...
- An avatar upload control in account settings: the browser resizes and re-encodes the image to a
  ...
  ([#176](https://github.com/Jolls/deckshare/issues/176)).
...
- Pressing Enter in a note field editor now inserts a `<br>` instead of a raw newline, so line
  ...
  ([#185](https://github.com/Jolls/deckshare/issues/185)).
...
- A "Preview" button on the note-editing forms (new note and edit note) that renders the note's
  ...
  anything ([#181](https://github.com/Jolls/deckshare/issues/181)).
...
- A persistent header bar on every authenticated page showing the signed-in account's display
  ...
  ([#177](https://github.com/Jolls/deckshare/issues/177)).
...
- A "Keep studying" button appears on the review-done screen once a deck's daily new/review cap
  ...
  ([#172](https://github.com/Jolls/deckshare/issues/172)).

### Changed
- The review-done screen's "Back to deck" link is now a white button, alongside a new blue "Back
  to home" button to its left ([#173](https://github.com/Jolls/deckshare/issues/173)).
...
- A "Study All" button on the deck list starts one mixed session drawing from every deck the
  ...
  ([#169](https://github.com/Jolls/deckshare/issues/169)).
...
- Vendored Alpine.js and a small `web/static/app.css` to make the frontend responsive on
  ...
  ([#166](https://github.com/Jolls/deckshare/issues/166)).

### Changed
- Destructive delete/revoke buttons (deck, note, note type, deck access) now ask for
  confirmation before submitting ([#166](https://github.com/Jolls/deckshare/issues/166)).
...
- Removing or reordering an existing field or template on a note type that already has notes no
  ...
  ([#89](https://github.com/Jolls/deckshare/issues/89)).
...
- Orphaned media is now collected by an hourly in-process sweep (`internal/media/gc.go`, beside
  ...
  ([#91](https://github.com/Jolls/deckshare/issues/91)).
...
- The "New" count on `/decks` and `/decks/{id}` is now capped to the deck's remaining daily
  ...
  ([#106](https://github.com/Jolls/deckshare/issues/106)).
...
- Playwright E2E suite (`tests/e2e/`): a real-browser spec drives signup, deck/note creation,
  ...
  server reports the event `applied` ([#100](https://github.com/Jolls/deckshare/issues/100)).
...
- Review Deck Prioritization (`priority` deck setting: due cards first, new cards first, or no
  ...
  behavior [#118](https://github.com/Jolls/deckshare/issues/118) asked for and
  [#115](https://github.com/Jolls/deckshare/issues/115) had left unbuilt. Replaces the "New/review
...
- `.apkg` import now persists a new card's Anki queue position (`cards.import_due_position`) and
  ...
  [#82](https://github.com/Jolls/deckshare/issues/82).

## [0.2.5] - 2026-08-29

### Added
- Per-deck `due.lookAheadMinutes` preset setting (default 0, matching the `due<=now` tightening
  in [#155](https://github.com/Jolls/deckshare/issues/155)): an opt-in window, up to 24h, that widens
  ...
  single scalar look-ahead the way the other three do) ([#154](https://github.com/Jolls/deckshare/issues/154)).
...
- Study-queue eligibility (`ListDueCardsForStudy`, `ListReviewCardsForStudy`, `CountQueueForDeck`,
  ...
  moved past it. See [#154](https://github.com/Jolls/deckshare/issues/154), filed for a future
...
- Deck access management, the first thing that grants a second user a `deck_access` row: a
  ...
  be left with nobody able to manage or delete it ([#83](https://github.com/Jolls/deckshare/issues/83)).
...
- `GET /media/{sha256}` now forces `Content-Type` to `application/octet-stream` for any stored
  ...
  script from running under a shared deck ([#133](https://github.com/Jolls/deckshare/issues/133)).
...
- Fixed `.apkg`/`.colpkg` import discarding the parsed FSRS desired retention, which scheduled
  ...
  a retention the user has since changed via `/settings`) ([#81](https://github.com/Jolls/deckshare/issues/81)).
## [0.2.0] - 2026-08-28
...
- Rewrote the run-app skill's `run.sh`/`reset-db.sh` bash scripts as a single Go program
  ...
  ([#109](https://github.com/Jolls/deckshare/issues/109)).
...
- Added a "Left today" figure to the deck list and deck page, showing how many cards are
  ...
  New/Learning/Due breakdown ([#137](https://github.com/Jolls/deckshare/issues/137)).
- Added the ability to change a note's note type from the note edit page, for note types with
  ...
  ([#138](https://github.com/Jolls/deckshare/issues/138)).
...
- Fixed a new card's `Good` rating repeating in the same study session, which forced users
  ...
  ([#136](https://github.com/Jolls/deckshare/issues/136)).
...
- Fixed DB-backed tests in `internal/auth` and `internal/http` that failed against a seeded
  ...
  ([#134](https://github.com/Jolls/deckshare/issues/134),
  [#119](https://github.com/Jolls/deckshare/issues/119),
  [#108](https://github.com/Jolls/deckshare/issues/108)).
...
- Simplified `internal/render`'s SVG-allowlist sanitisation: collapsed four duplicated
  ...
  copies — no behavior change ([#127](https://github.com/Jolls/deckshare/issues/127)).
- Simplified the `internal/http` CRUD/import surface plus `auth.go`/`review.go`: added
  ...
  behavior change ([#128](https://github.com/Jolls/deckshare/issues/128)).
...
- Simplified `internal/apkg`'s reader/writer: consolidated error wrapping, deduped zip-member and
  ...
  change ([#124](https://github.com/Jolls/deckshare/issues/124)).
- Simplified `internal/fsrs`: named the supported parameter-count constant, deduped
  ...
  `go-fsrs`'s own default instead of a hardcoded literal ([#125](https://github.com/Jolls/deckshare/issues/125)).
- Simplified `internal/review` and `internal/http/review.go`: renamed `newlimit.go` to `preset.go`
  ...
  ([#126](https://github.com/Jolls/deckshare/issues/126)).
- Introduced `review.ParamsCache`, a shared per-deck FSRS-params memoizer, and moved `GradeBatch`'s
  ...
  cache — no behavior change ([#124](https://github.com/Jolls/deckshare/issues/124),
  [#126](https://github.com/Jolls/deckshare/issues/126)).

### Fixed
- `internal/fsrs`'s `LearningSteps` conversion from the underlying library now saturates instead
  of silently wrapping on an out-of-range value ([#125](https://github.com/Jolls/deckshare/issues/125)).
- Corrected a misleading `resolveHomeDecks` warning message and the
  ...
  ([#124](https://github.com/Jolls/deckshare/issues/124), [#126](https://github.com/Jolls/deckshare/issues/126)).

### Removed
- Deleted the orphaned `GetCard` query and the unused `takePending` export from
  `window.enshuReview` ([#124](https://github.com/Jolls/deckshare/issues/124),
  [#126](https://github.com/Jolls/deckshare/issues/126)).
...
- Changing a password now invalidates every session for that account and reissues the acting
  ...
  ([#123](https://github.com/Jolls/deckshare/issues/123)).
- Added `X-Content-Type-Options: nosniff` and `Referrer-Policy: same-origin` alongside the
  existing CSP ([#123](https://github.com/Jolls/deckshare/issues/123)).
...
- Deleted five never-called, unscoped `SELECT * WHERE id = $1` getters (`GetDeck`,
  ...
  ([#122](https://github.com/Jolls/deckshare/issues/122)).
...
- Deduplicated repeated markup across templates into shared partials (`errorMsg`/`successMsg`,
  ...
  ([#129](https://github.com/Jolls/deckshare/issues/129)).
- Removed the standalone `go vet ./...` step from CI; golangci-lint's `standard` preset already
  runs `govet` ([#129](https://github.com/Jolls/deckshare/issues/129)).
...
- Per-deck daily review-card limit (`rev.perDay`, default 200), enforced independently of the
  ...
  ([#115](https://github.com/Jolls/deckshare/issues/115)).
- Configurable per-deck review order (`rev.order`: due date, random, ascending/descending
  ...
  ([#116](https://github.com/Jolls/deckshare/issues/116)).
...
- `ORIGIN` now accepts a comma-separated list of allowed origins, instead of exactly one. A
  ...
  ([#111](https://github.com/Jolls/deckshare/issues/111))
...
- Per-deck daily new-card limit: the reviewer's queue now caps never-seen cards at the deck's
  ...
  ([#101](https://github.com/Jolls/deckshare/issues/101))
...
- One-command local test-DB reset: `.claude/skills/run-app/reset-db.sh` wipes the `pgdata`
  ...
  ([#95](https://github.com/Jolls/deckshare/issues/95))
...
- New users are seeded with stock **Basic** and **Cloze** note types on signup, matching Anki's
  ...
  blocks or fails signup itself ([#97](https://github.com/Jolls/deckshare/issues/97))
## [0.1.21] - 2026-08-15
...
- `GET`/`POST /import/ai`: paste-in import for AI-generated cards — pick a deck + note type,
  ...
  ([#85](https://github.com/Jolls/deckshare/issues/85))
...
- `/decks` and `/decks/{id}` show a New/Learning/Due queue summary, using the same eligibility
  ...
  ([#80](https://github.com/Jolls/deckshare/issues/80))
...
- Import page's file picker now accepts `.colpkg` as well as `.apkg` — the reader and handler
  ...
  dialog ([#78](https://github.com/Jolls/deckshare/issues/78))
...
- `GET`/`POST /settings/fsrs` and `POST /decks/{id}/settings/fsrs`: set the desired-retention
  ...
  deck matches no rows and gets a 404 ([#63](https://github.com/Jolls/deckshare/issues/63))
...
- `GET`/`POST /import`: upload a `.apkg`, synchronous `read -> IR -> db`, redirect to the deck
  ...
  ([#62](https://github.com/Jolls/deckshare/issues/62))
...
- `internal/media`: filesystem-backed, content-addressed blob store
  ...
  never observes a partial write ([#60](https://github.com/Jolls/deckshare/issues/60))
- `.apkg` import now writes media into the blob store and records `media_blobs`/`media_refs`
  ...
  files to individual decks) ([#60](https://github.com/Jolls/deckshare/issues/60))
- `GET /media/{sha256}`: serves a blob to any user with `can_view` on a deck that references it,
  with long-lived private cache headers ([#60](https://github.com/Jolls/deckshare/issues/60))
...
- `internal/apkg/`: a database → IR → `.apkg` writer (`Export` + `Write`/`WriteFile`), completing
  ...
  schema 18 stays reader-only and gated behind [#61](https://github.com/Jolls/deckshare/issues/61)),
  ...
  package-wide creation instant ([#59](https://github.com/Jolls/deckshare/issues/59))
...
- `internal/apkg/`: `.apkg`/`.colpkg` reader (legacy zip and modern zstd containers, schema-11
  ...
  [#61](https://github.com/Jolls/deckshare/issues/61)) and an IR → database writer, idempotent on
  `(owner_id, guid)`, filing each card's `deck_id` from that card's own home deck rather than the
  note's (architecture.md §20) ([#58](https://github.com/Jolls/deckshare/issues/58))
- `review.LockCards`, an exported per-`(user, card)` advisory-lock helper shared by the importer
  and `GradeBatch` ([#58](https://github.com/Jolls/deckshare/issues/58))
- Synthetic in-memory `.apkg` fixture builders (`internal/apkg/synthetic_test.go`) covering
  ...
  ([#58](https://github.com/Jolls/deckshare/issues/58))
...
- `.apkg` reading enforces archive ceilings (member count, per-member and total decompressed
  ...
  cannot exhaust memory ([#58](https://github.com/Jolls/deckshare/issues/58))
...
- The reviewer: `internal/review/` (batch precompute over `internal/fsrs`, server-authoritative
  ...
  ([#56](https://github.com/Jolls/deckshare/issues/56))
- `POST /api/reviews/batch`'s route-level test asserting the client cannot write scheduling
  ...
  test-priority item in the repo (CLAUDE.md §10.1) ([#56](https://github.com/Jolls/deckshare/issues/56))
- The reviewer's client-side queue module (`web/static/review.js`, vanilla JS, no build step):
  ...
  route, rather than a CDN ([#56](https://github.com/Jolls/deckshare/issues/56))
...
- Deck / note-type / note / card CRUD: `internal/http/{decks,notetypes,notes}.go` handlers and
  ...
  ([#54](https://github.com/Jolls/deckshare/issues/54))
- Card generation on note create and diff-based card regeneration on note edit
  ...
  docs/schema.md warns against ([#54](https://github.com/Jolls/deckshare/issues/54))
- `internal/render.ClozeOrdinals`, a pure cloze-marker scanner shared by card generation and the
  future rendering engine (#55) ([#54](https://github.com/Jolls/deckshare/issues/54))
- DB-backed regression test for the card-regeneration trap (edit a 3-cloze note; assert a
  ...
  for `notes.owner_id = decks.owner_id` ([#54](https://github.com/Jolls/deckshare/issues/54))
- `internal/render/`: note-type template rendering (§8) — `{{Field}}`, `{{#Field}}`/`{{^Field}}`
  ...
  sole entry point ([#55](https://github.com/Jolls/deckshare/issues/55))
- Card-content HTML sanitisation on render, via a `bluemonday` allowlist policy: no
  ...
  blob ([#55](https://github.com/Jolls/deckshare/issues/55))
- `internal/render.SanitiseCSS`: note-type CSS-blob sanitisation via `douceur`'s parser —
  ...
  selectors scoped to `.enshu-card` ([#55](https://github.com/Jolls/deckshare/issues/55))
- `{{type:Field}}` answer-input boundary: rendering leaves an unguessable placeholder (never
  ...
  *after* sanitisation, so the widget can never be sanitised as card content and never needs a
  hole in the allowlist ([#55](https://github.com/Jolls/deckshare/issues/55))
...
- `GET /` now redirects an authed user straight to `/decks` instead of rendering a placeholder
  home page; `web/templates/home.html` removed ([#54](https://github.com/Jolls/deckshare/issues/54))
- `internal/render.ClozeOrdinals`'s cloze-marker scanner now tracks brace depth, so a nested
  ...
  ([#55](https://github.com/Jolls/deckshare/issues/55))
...
- `/settings` account routes: profile (display name, timezone, day-start hour) and password
  change ([#52](https://github.com/Jolls/deckshare/issues/52))
- First server-rendered HTML templates (`web/templates/`): login, signup, home, and settings
  pages over a shared layout, Pico CSS from CDN ([#52](https://github.com/Jolls/deckshare/issues/52))
- `internal/fsrs/`: pure wrapper over `go-fsrs` v4 (FSRS-6) with `Schedule` (grade-time recompute)
  ...
  calendar-day-boundary semantics ([#53](https://github.com/Jolls/deckshare/issues/53))
- Property-based batch-preview/grade-time consistency test (CLAUDE.md §10.2, this repo's
  ...
  `Schedule` and full-sequence replay ([#53](https://github.com/Jolls/deckshare/issues/53))
...
- Rate limiting on `POST /settings/password`, keyed per-user, closing a brute-force gap on
  `current_password` that the initial pass missed ([#52](https://github.com/Jolls/deckshare/issues/52))
- Login's per-email rate limit now keys on the lower-cased address, matching the case-insensitive
  ...
  per variant ([#52](https://github.com/Jolls/deckshare/issues/52))

## [0.1.7] - 2026-08-11

### Added
- Full schema as 14 `goose` migrations, one per table, and `sqlc`-generated models/queries
  ([#50](https://github.com/Jolls/deckshare/issues/50))
...
- `compose.yaml`'s dev database bumped `postgres:16` → `postgres:18`, enabling native
  ...
  application-generated id ([#50](https://github.com/Jolls/deckshare/issues/50))
...
- Go module scaffold: `cmd/enshu/` entrypoint (config from env, DB pool, graceful shutdown) and
  the `internal/` package skeleton from architecture.md §4 ([#49](https://github.com/Jolls/deckshare/issues/49))
...
### Changed
- Grading is server-authoritative in the code, not only in the docs (CLAUDE.md §2.7). A client now asserts `{ id, cardId, rating, reviewedAt, durationMs }` and nothing else; `applyReviewBatch` reads the stored `user_card_state`, schedules the grade itself through `$lib/fsrs`, and writes *its own* numbers to `review_log` and `user_card_state` ([#39](https://github.com/Jolls/deckshare/issues/39))
- The client's local FSRS result travels as `predicted` — compared against the server's answer, never stored, never authoritative. §2.6 is unchanged: grading is still synchronous and local, and the UI never awaits the network ([#39](https://github.com/Jolls/deckshare/issues/39))
- `/api/reviews/batch` responds with the server's post-review state per event, so a client can reconcile its queue ([#39](https://github.com/Jolls/deckshare/issues/39))
- `review_log`'s FSRS columns come off `ts-fsrs`' own review log rather than being projected from the client's `stateBefore`/`stateAfter` ([#39](https://github.com/Jolls/deckshare/issues/39))
- Write-queue batches are sorted by review time before scheduling, and an event already present in `review_log` is skipped outright — a retry no longer reschedules from the row it advanced ([#39](https://github.com/Jolls/deckshare/issues/39))
- A batch whose reviews predate the stored `last_review` is replayed from `review_log` rather than folded onto the stored row, so out-of-order arrival converges instead of recording a `*_before` the card was never in ([#39](https://github.com/Jolls/deckshare/issues/39))

### Security
- `applyReviewBatch` takes a per-`(user, card)` advisory lock for the transaction. Scheduling server-side makes each grade a read-modify-write, and two concurrent batches for one card (two tabs, a redelivered POST) would otherwise both schedule from the same state and silently drop a review ([#39](https://github.com/Jolls/deckshare/issues/39))
- `/api/reviews/batch` refuses a `reviewedAt` more than five minutes ahead of server time. Unbounded it both poisons the server's schedule and writes a `last_review` far enough ahead to freeze that card behind the §6 guard permanently ([#39](https://github.com/Jolls/deckshare/issues/39))
- The retry-skip lookup is global rather than scoped to `user_id`, matching `review_log.id`'s primary key. Scoped, a colliding id would have been dropped by `ON CONFLICT` while its `user_card_state` write still landed — state with no backing log row ([#39](https://github.com/Jolls/deckshare/issues/39))

### Added
- `src/lib/server/fsrs/divergence.ts`: the §6 comparison (`state`/`reps`/`lapses` exact, `stability`/`difficulty` within `1e-6`, `due` to the second), a structured log line naming both `fsrs_version`s, and a process-lifetime counter. No table — the expected rate is zero, so it is an alert condition ([#39](https://github.com/Jolls/deckshare/issues/39))
- CLAUDE.md §10.1 coverage, the repo's top testing priority: a grade whose `predicted` block is hostile rather than merely stale stores the server's value, keeps `review_log` clean, and raises one divergence per event ([#39](https://github.com/Jolls/deckshare/issues/39))

### Removed
- The write queue's `localStorage` durability. It existed to serve offline study, which architecture.md §11 rules out; the idempotent endpoint and client-generated event ids stay ([#39](https://github.com/Jolls/deckshare/issues/39))
...
- architecture.md §12 no longer claims the media store is implemented in `src/lib/server/media/store.ts` — that path does not exist and [#34](https://github.com/Jolls/deckshare/issues/34) is open
...
## [0.1.2] - 2026-08-08
### Added
- Isomorphic `lib/fsrs/` wrapper over `ts-fsrs` — pure functions, no DB, no `fetch`, no browser globals, so client and server schedule identically ([#5](https://github.com/Jolls/deckshare/issues/5))
- `lib/render/` note-type template engine: `{{Field}}`, `{{#Field}}`/`{{^Field}}` conditionals, `{{FrontSide}}`, filters, and cloze fan-out by ordinal ([#12](https://github.com/Jolls/deckshare/issues/12))
- Auth + accounts: `argon2id` password hashing, sessions keyed by the SHA-256 hash of the cookie token, `Origin`-header CSRF checks, and login rate limiting ([#7](https://github.com/Jolls/deckshare/issues/7))
- Deck / note-type / note / card CRUD query layer and JSON API routes, with `deck_access` authorisation enforced at the query layer ([#8](https://github.com/Jolls/deckshare/issues/8))
- `.apkg` / `.colpkg` reader: package → intermediate representation, covering collection schemas 11 and 18+, the legacy and zstd containers, and content-addressed media ([#9](https://github.com/Jolls/deckshare/issues/9))
- Synthetic `.apkg` fixtures built to `docs/apkg-format.md`, pending real Anki exports ([#9](https://github.com/Jolls/deckshare/issues/9))
- Collection-member preference regression test: a package carrying both `collection.anki21` and a one-note `collection.anki2` downgrade stub must read the former ([#9](https://github.com/Jolls/deckshare/issues/9))
- The reviewer and the write queue: one-request session start, synchronous local grading, a durable `localStorage` queue draining to an idempotent `POST /api/reviews/batch`, and the server-side `review_log` recompute path ([#13](https://github.com/Jolls/deckshare/issues/13))
- `defaultFsrsParams()` — the `ts-fsrs` defaults a user schedules with before their first optimiser fit ([#13](https://github.com/Jolls/deckshare/issues/13))
- Browser-facing signup, login, and logout pages wrapping the existing `/(auth)/*` JSON endpoints, with client-side validation mirroring the server's email/password rules ([#28](https://github.com/Jolls/deckshare/issues/28))
- Deck list/create, an auto-seeded default "Basic" note type, add-note form, and note list/delete pages under `src/routes/(app)/decks/` — the minimum UI to create study material without `curl` ([#29](https://github.com/Jolls/deckshare/issues/29))
- Re-import dedup keys: `UNIQUE (owner_id, name)` on `decks` and `note_types`, `notes.owner_id` with `UNIQUE (owner_id, guid)`, and `review_log.anki_id` with `UNIQUE (user_id, card_id, anki_id)` ([#32](https://github.com/Jolls/deckshare/issues/32))
- `decks.anki_id`, for `.apkg` round-trip export fidelity ([#32](https://github.com/Jolls/deckshare/issues/32))
- `DuplicateNameError` → HTTP 409 on the deck and note-type create/rename endpoints, now that a name is a uniqueness key ([#32](https://github.com/Jolls/deckshare/issues/32))

### Changed
- Note identity is per owner, not per deck: `notes (deck_id, guid)` → `notes (owner_id, guid)`, with a plain `notes (deck_id)` index kept for deck-scoped queries. Amends the CLAUDE.md §2.2 invariant — `guid` is globally unique in Anki, so a deck-scoped key made note idempotency depend on deck dedup landing first ([#32](https://github.com/Jolls/deckshare/issues/32))
- Deck and note-type re-import dedup on name rather than `anki_id`: Anki ids are per-collection (deck id 1 is `Default` in every collection), so an `anki_id` key would silently merge two unrelated imported decks and two different note types ([#32](https://github.com/Jolls/deckshare/issues/32))

### Fixed
- `docs/apkg-format.md`: `cards.due` has three meanings discriminated by `queue`, not two by `type`, and `cards.odue` shadows it entirely for cards in a filtered deck; the negative-`ivl` seconds encoding also applies to `revlog`; schema-18 configs are protobuf; deck-name separators and field ordering differ per schema; the modern media index is protobuf ([#9](https://github.com/Jolls/deckshare/issues/9))

### Security
- Card content is sanitised on render — note HTML is user-authored and, under shared decks, *other users'* HTML ([#6](https://github.com/Jolls/deckshare/issues/6))
- `.apkg` reading enforces archive ceilings (member count, per-member and total decompressed size) and checks a zstd frame's declared size before decompressing, so an uploaded package cannot exhaust memory ([#9](https://github.com/Jolls/deckshare/issues/9))

## [0.1.1] - 2026-08-07
### Added
- Full data model in Drizzle: content tables, `deck_access`, `user_card_state`, append-only `review_log`, `user_fsrs_params`, and content-addressed media ([#4](https://github.com/Jolls/deckshare/issues/4))
- `studyDayStart()` / `studyDayEnd()` — per-user 04:00-local study-day boundary computed in SQL ([#4](https://github.com/Jolls/deckshare/issues/4))
- Isomorphic `uuidv7()` for client-generated, time-ordered ids ([#4](https://github.com/Jolls/deckshare/issues/4))

### Removed
- The scaffold's placeholder `task` table ([#4](https://github.com/Jolls/deckshare/issues/4))

## [0.1.0] - 2026-08-07
### Added
- SvelteKit + TypeScript (strict) scaffold: ESLint, Prettier, Vitest, Playwright, Drizzle + PostgreSQL, CI ([#3](https://github.com/Jolls/deckshare/issues/3))
